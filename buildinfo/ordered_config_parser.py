...
# TODO - Convert to super class of ConfigParser
...
class ConfigParser:
    ...
    def has_section(self, section):
        """Indicate whether the named section present in the configuration.
    ...
    def write(self, fp):
        ...
        for section in self.__sections.keys():
            fp.write("[%s]\n" % section)
            for (key, value) in self.__sections[section].iteritems():
                if key != "__name__":
                    fp.write("%s = %s\n" %
                             (key, str(value).replace('\n', '\n\t')))
            fp.write("\n")
